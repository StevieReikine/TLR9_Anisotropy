...
# import data that will be used for fitting
data = np.loadtxt("mTLR9_1668_dataonly.txt")
data_x = data[:, 0]  # x values of data (P0 (nM))
data_y = data[:, 1]  # y values of data (measured Anisotropy (mA))
...
D0 = 5  # set to experimental value of D0

# plot data
plt.plot(np.log10(data_x), data_y, 'b-', label= 'data')

# fit K1 and K4 to data
popt, pcov = curve_fit(Anisotropy, data_x, data_y, p0 = [7.0, 50.0])
print("K1 is: ", popt[0], ", and K4 is: ", popt[1])

# generate new Y for fit K1, K4
Y = Anisotropy(data_x, popt[0], popt[1])

# plot data as generated with best fit of K1, K4- with enough points in X this could be close to a line
plt.plot(np.log10(data_x), Y, 'r-', label= 'fit')
plt.show()

# save data generated by best fit of K1 and K4 to csv file
np.savetxt('Kd-test-K1-K4_fitted_f.csv', (data_x, Y), delimiter=',')
